...
import 'package:tomate/tomate_painter.dart';
import 'package:flutter_ringtone_player/flutter_ringtone_player.dart';

class TomateApp extends StatelessWidget {
  const TomateApp({super.key});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: TomateAppleTimer(),
    );
  }
}

class TomateAppleTimer extends StatefulWidget {
  const TomateAppleTimer({super.key});

  @override
  State<TomateAppleTimer> createState() => _TomateAppleTimerState();
}

class _TomateAppleTimerState extends State<TomateAppleTimer> {
  // Default settings
  int workMinutes = 25;
  int breakMinutes = 5;
  ...
  String formatTime(Duration d) => d.toString().split('.').first.padLeft(8, "0").substring(3);
      ...
      builder:
          (context) => StatefulBuilder(
            builder: (context, setStateDialog) {
              return AlertDialog(
                title: Text('Configurações do Tomate'),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (localAlarmEnabled)
                        Column(
                          children: [
                            ListTile(title: Text('Tipo de som'), contentPadding: EdgeInsets.symmetric(horizontal: 0)),
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(alarmEnabled ? 'Som ativado (${getAlarmTypeIcon()})' : 'Som desativado'),
        ...
        children: [
          ...
          Center(
            child: Column(
              ...
              children: [
                Text(getModeText(), style: TextStyle(fontSize: 28, color: Colors.white, fontWeight: FontWeight.w500)),
                Text(
                  formatTime(remaining),
                  style: const TextStyle(fontSize: 72, color: Colors.white, fontWeight: FontWeight.bold),
                ),
                Text('Ciclo: $cycle / $targetCycles', style: const TextStyle(color: Colors.white, fontSize: 30)),
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    ElevatedButton(
                ...
                Row(
                  ...
                  children: [
                    TextButton.icon(
                      ...
                      label: Text('Editar Configurações', style: TextStyle(color: Colors.white)),
                    ),
                  ],
                ),
              ],
